...
namespace RacingGameKit
{
    ...
    public class Racer_Register : MonoBehaviour
    {
            ...
            if (IsRacerStarted && !IsRacerFinished)
            {
                ...
                if (IsCheckPointSystemEnabled)
                {
                    ...
                    if (Trigger.tag == "CheckPoint" || !RacerDetail.RacerFinished)
                    {
                        ...
                        sectorSpeedAndTime.SectorSpeed = base.rigidbody.velocity.magnitude * 3.6f;
        ...
        private void Update()
        {
            ...
            ///Feedback Wrong Way
            if (Mathf.Abs(AngleDif) <= 230f && Mathf.Abs(AngleDif) >= 120f && (rigidbody.velocity.magnitude * 3.6f) > 10)
            {
                IsWrongWay = true;
        ...
        private float CalculateRemainingDistance()
        {
            ...
            if (Mathf.Abs(AngleDif) <= 270f && Mathf.Abs(AngleDif) >= 80)
            { RotatedBack = true; }
            else
            { RotatedBack = false; }
